...
void Game::init()
{
    ...
    if (!m_mainMusic.openFromFile("ASSETS/Music/Village.ogg"))
    {
        std::cout << "music failed to load\n";
    }
    m_mainMusic.setVolume(10);
    m_mainMusic.play();

    if (!m_goodEnding.openFromFile("ASSETS/Music/22 - Dream.ogg"))
    {
        std::cout << "good ending music failed to load\n";
    }
    m_goodEnding.setVolume(80);
    if (!m_badEnding.openFromFile("ASSETS/Music/14 - Curse.ogg"))
    {
